...
int main ()
{
	...
	ifstream ifst("CET4.txt");
	ifstream from_old("worddata_zjl_2.txt");
	ofstream ferr("worddata_zjl_3.txt");
	...
	while (getline(ifst, s))
	{
		...
		if (s.size() > 2)
		{
			...
			if (sign == 1)
			{
				word -> word = s.substr(start, signal);
			...
			if (sign == 3)
			{
				...
				if (!has)
				{
					word->tagList.push_back(WORD_TAG[2]);
				}
