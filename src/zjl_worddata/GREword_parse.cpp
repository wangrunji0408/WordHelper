...
int main ()
{
	...
	ifstream ifst("GRE.txt");
	ifstream from_old("worddata_zjl_4.txt");
	ofstream ferr("worddata_zjl_5.txt");
	...
	while (getline(ifst, s))
	{
		...
		if (s.size() > 2)
		{
			...
			if (sign == 3)
			{
				...
				if (!has)
				{
					word->tagList.push_back(WORD_TAG[5]);
				}
